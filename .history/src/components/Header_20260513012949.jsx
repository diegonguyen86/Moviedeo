      ...
      <div className="flex items-center justify-between px-6 h-full max-w-container-max mx-auto">
        ...
        <nav className="hidden md:flex items-center gap-10">
          ...
          <Link 
            ...
            className={`text-[15px] font-bold transition-all hover:text-primary ${isActive('phim-chieu-rap') ? "text-primary border-b-2 border-primary" : "text-text-secondary"}`}
          >
            Phim Chiếu Rạp
          </Link>
      ...
      {isMobileMenuOpen && (
        <nav className="md:hidden absolute top-full left-0 w-full bg-black/95 backdrop-blur-xl border-b border-border-glass flex flex-col px-6 py-4 gap-4 shadow-2xl">
          ...
          <Link 
            ...
            state={{ type: 'danh-sach', slug: 'phim-chieu-rap', title: 'Phim Chiếu Rạp' }}
            onClick={closeMobileMenu}
            className={`text-[16px] font-bold py-2 ${isActive('phim-chieu-rap') ? "text-primary" : "text-text-secondary"}`}
          >
            Phim Chiếu Rạp
          </Link>
